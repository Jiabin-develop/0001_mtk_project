...
{
    cy_rslt_t rst_write;
    rst_write = i2c_write(obj->_ui2c, MAX30102_I2C_ADDR, reg, 1, buf, buflen, MAX30102_I2C_TIMEOUT);
    // tm_printf((UB *)" max30102_write result = %d\n", rst_write);
}
...
{
    i2c_read(obj->_ui2c, MAX30102_I2C_ADDR, reg, 1, buf, buflen, MAX30102_I2C_TIMEOUT);
}
...
void max30102_read_fifo(max30102_t *obj)
{
    ...
    max30102_read(obj, MAX30102_INTERRUPT_STATUS_1, &interrupt_status_1, 1);
}

uint32_t max30102_read_fifo_N(max30102_t *obj, uint32_t N, uint32_t *buf, int32_t *buf_start, int32_t *buf_end, uint32_t buf_size)
{
    // First transaction: Get the FIFO_WR_PTR
    uint8_t wr_ptr = 0, rd_ptr = 0;
    max30102_read(obj, MAX30102_FIFO_WR_PTR, &wr_ptr, 1);
    ...
    for (int8_t i = 0; i < num_samples; i++)
    {
        ...
        {
            if (next_end > *buf_start)
            {
                buf[next_end] = red_sample;
                *buf_end = next_end;
            }
            else
            {
        ...
        // tm_printf((UB *)"{ppg_red}%d\n", red_sample);
        //  tm_printf((UB *)"{buf_start}%d, {buf_end}%d\n", *buf_start, *buf_end);
    }
